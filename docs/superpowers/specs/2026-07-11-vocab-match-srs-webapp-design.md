...
### 5. 複習 session 控制：`web/js/session.js`
- **三種模式共用同一套 SRS 佇列**：session 取 `srs.dueQueue()` 產生本次要複習的字，依使用者所選**模式**呈現；每張卡複習完，該模式回傳一個 grade（Again/Hard/Good/Easy），session 即呼叫 `srs.review()` → `store.save()`。
- 支援兩種節奏：**逐盤**（配對消除一次 6 字）與**逐張**（打字、四選一一次 1 字）。
- 相依：srs.js、store.js、所選 mode、audio.js。UI 提供**模式切換**（三顆分頁/晶片）。

### 5a. 模式 A — 配對消除：`web/js/modes/match.js`
- 一盤 6 對＝12 張卡，響應式格線（手機 3 欄×4 列）打散；卡片為「單字卡（漢字＋假名）」或「意思卡（中文）」，6 字取自佇列。
- 點兩張卡，構成同字 word↔meaning → 消除（彈開＋粒子＋音效＋連擊＋1）；否則抖動、連擊歸零。清盤放彩帶再補下一盤。
- **grade（每字）**：消除前曾配錯 → `Again`；未配錯但 > 8 秒 → `Hard`；未配錯正常 → `Good`；未配錯且 < 2.5 秒且首選即命中 → `Easy`。

### 5b. 模式 B — 打字競速：`web/js/modes/typing.js`
- 顯示單字（漢字，可設定是否遮住假名）＋計時；使用者輸入**讀音**（羅馬拼音）。
- 比對：把輸入與該卡 `romaji` 正規化後比較（`ō≈ou≈oo`、`ū≈uu`、忽略大小寫/空白/`'`）；也接受直接輸入假名比對 `kana`。
- **grade**：首次即正確且 < 4 秒 → `Easy`；正確無退格 → `Good`；正確但有退格/錯字或 > 8 秒 → `Hard`；答錯或按「看答案」→ `Again`。
- juice：正確逐字亮綠、連擊、WPM/連續正確顯示；錯誤紅閃。

### 5c. 模式 C — 四選一快答：`web/js/modes/quiz.js`
- 顯示單字，四個中文意思選一（1 正解 ＋ 3 干擾，干擾優先取同級別/同詞性者）；點選作答。
- **grade**：答對且 < 1.5 秒 → `Easy`；答對 → `Good`；答對但 > 5 秒 → `Hard`；答錯 → `Again`。
- juice：正解卡放大＋綠光＋音效，錯誤紅閃並標出正解。

> 每個 mode 是獨立單元：輸入＝一批（或一張）卡，輸出＝每卡一個 grade。可各自單測。
...
- 頂部：**模式切換（配對／打字／四選一）**＋級別晶片（N5–N1）＋主題晶片（依所選級別動態產生）＋「今日到期／新字」計數。
...
3. 開局：`srs.dueQueue()` 產生 session 佇列 → 依所選模式呈現（配對逐盤 6 字／打字・四選一逐張）。
4. 作答：模式判定 grade → `srs.review()` → `store.save()` → `sync` debounce `PUT`。
...
- **三種模式的 grade 對應**：各自以模擬事件驗證映射（配對是否曾配錯/耗時；打字正確/退格/超時；四選一對錯/耗時）。
- 打字比對正規化：`ō/ou/oo`、`ū/uu`、大小寫/空白/`'` 的等價測試。
- 手動：實機（手機＋電腦）三種模式各跑一輪、設同一密碼驗證同步合一。

## v1 範圍（YAGNI）

**做**：三種模式（配對消除／打字競速／四選一快答）共用 SM-2 SRS、模式切換、級別/主題篩選、密碼同步、fancy 音效/特效、分數連擊、設定面板。
**不做（之後）**：翻卡自評模式、發音 TTS、統計儀表板、排行榜、間隔學習的細緻參數 UI、四選一「意思→單字」反向題。
