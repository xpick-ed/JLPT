...
- 2026-07-18 單字/文法覆蓋補到 100%＋全資料對抗式總審（PWA v33，167/167 測試全過）:
  ...
  - 2026-07-18 功能盤點: 修一個真 bug（筆順模式只載入已選級別的筆順檔，但漢字資料以「最低出現級別」去重存放，導致單選 N1 時只有 18% 詞可描紅；改成一律載入全五級聯集，18%→87%，sw v34）。盤點結果：動詞變位 N1 覆蓋 34%／同音辨析 5% 皆非缺陷，是 N1 詞彙組成（動詞類詞性僅占 34%）與日語讀音碰撞率（生僻詞彙同音率天生低，全五級合併也只有 9%）的真實反映，非資料或程式問題。N1 分類 chip 增至 64 個、PWA 預快取增至 6.16MB，為手機體驗待觀察項目（非阻塞）。
