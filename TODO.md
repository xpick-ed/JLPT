...
- 2026-07-18 單字/文法覆蓋補到 100%＋全資料對抗式總審（PWA v33，167/167 測試全過）:
  - 單字累計詞彙量三級全數達成或超過官方公認目標：N3 3,208→**3,750**（100.0%）、N2 4,966→**6,000**（100.0%）、N1 6,729→**10,079**（100.8%，略超）。共新增 3,350 詞，兩輪 workflow（~150 agent：generate+獨立verify）生成、本地驗證（詞性白名單、振假名格式含々踊り字與半形括號正規化、詞幹可還原、跨全庫去重）後入庫。
  - N1 文法句型 160→**218**（+58，含配套 cloze/排序題；App 內 N1 cloze 108→166、排序 98→156）。
  - 全資料對抗式總審：對這批新增 3,350 詞+58 句型跑獨立於生成流程之外的複審（24 agent），揪出 9 處（5 詞性、1 羅馬字、2 例句翻譯、0 文法問題），確認生成品質已高。加上先前 N1 舊庫審稿（25 處），全部新舊資料至此皆經過至少一輪對抗式審稿。
  - 衍生資料同步：聲調（kanjium）補查達 84.0% 覆蓋（8,465/10,079）；筆順（KanjiVG）新漢字 277 個全數補齊（1,940→2,217，0 缺）；N3/N2/N1 單字書 PDF＋N1 文法句型書 PDF 皆已重建；JLPT_N5-N1_單字合併.csv 已重建（10,079 列）。
  - ~~已知缺口：兩份 Anki CSV 未更新~~ 使用者不用 Anki，不用處理，維持現狀即可。
  - 2026-07-18 功能盤點: 修一個真 bug（筆順模式只載入已選級別的筆順檔，但漢字資料以「最低出現級別」去重存放，導致單選 N1 時只有 18% 詞可描紅；改成一律載入全五級聯集，18%→87%，sw v34）。盤點結果：動詞變位 N1 覆蓋 34%／同音辨析原 5% 皆非缺陷，是 N1 詞彙組成（動詞類詞性僅占 34%）與日語讀音碰撞率的真實反映。
  - 2026-07-18（追加）同音辨析跨級比對＋分類 chip 手機體驗（sw v36，168/168 測試）: ①同音辨析改對照全五級合併詞庫找搭檔（app.js ensureAllVocab() 背景載入），N1-only 覆蓋 5%→8%（實測 枠組/N1↔枠組み/N2、真髄/N1↔神髄/N2 等原本配不到搭檔的組合現在正確跨級配對）②分類 chip 從單行橫向捲動（N1 曾溢出 12,540px）改為自動換行＋預設收合 10 個＋展開/收起按鈕＋即時搜尋框（>10 個分類才顯示，輸入時保持游標焦點不中斷），全部級別皆適用。
- ~~待審稿~~ 已完成（2026-07-17）: N1 單字/文法補跑對抗式審稿（2 輪 ×15 reviewer+verifier agents，取聯集）——修正 25 處：刪除幻覺詞 黄ずむ→換成 赤らむ、9 個詞性、6 句例句、1 個中譯、3 條文法接續。
- 網頁學習道場: 已完成（web/，單字六模式＋文法兩模式＋閱讀＋SM-2 SRS＋Google 登入同步）。本機試玩：`npm run dev`。
  - 2026-07-17（第二批）核心升級 7 項（PWA v31，162/162 JS 測試全過，皆 Playwright 實測）:
    ① FSRS-4.5 排程取代 SM-2（srs.js 全換、API 不變；卡片新增 s/d 欄位；舊卡下次複習時平滑遷移 s≈interval、d 由 ease 推估；ease 續維護供弱點判定；again 保留 10 分鐘重學）
    ② 保留率統計（daily bucket 新增 rev/revOk＝舊卡回憶嘗試/成功；🏅 面板顯示近 30 天保留率長條＋總保留率＋FSRS 難度分佈五段條）
    ③ 一鍵今日課表（教練面板「▶ 開始今日課表」：單字混合≤40→弱點≤15→文法 due≤15→聽力10 自動接續；浮動 HUD 顯示段落/剩餘/可✕退出；手動切分頁自動取消；完成 🎓+confetti）
    ④ 混合模式（特訓預設 tab「混合」：每卡隨機出 四選一/例句/助詞/聽力 題型；課表單字段用它）
    ⑤ 備份/匯入（設定面板：匯出全 state JSON 下載；匯入走 mergeStates 合併不覆蓋，壞檔 toast）
    ⑥ 首次導引（無卡片且未 onboarded 顯示：選級別/考期/每日目標＋分頁導覽；settings.onboarded）＋手機打磨（tab scroll-snap、48px 觸控目標）
    ⑦ N1 補審（見上）
