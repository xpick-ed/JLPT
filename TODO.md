...
- 2026-07-18 單字/文法覆蓋補到 100%＋全資料對抗式總審（PWA v33，167/167 測試全過）:
  ...
  - **已知缺口**：兩份 Anki 匯入用 CSV（`JLPT_N5-N1_單字合併_Anki假名.csv`、`JLPT_N5-N1_文法句型合併_Anki.csv`）用真正的日語斷句分詞（如「五[ご] 時[じ]に」bunsetsu 空格），需要 MeCab/fugashi 等形態素分析工具才能正確重建，本機無此工具故未重建（維持擴充前的舊內容，非壞檔，只是不含新詞）。有需要時另外處理。
